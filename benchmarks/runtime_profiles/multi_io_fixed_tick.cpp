...
Config parseConfig(int argc, char* argv[]) {
    if (argc > 4) {
        throw std::invalid_argument(
            "usage: gamenet_multi_io_fixed_tick_benchmark "
...
int run(const Config& config) {
    ...
    gamenet::examples::MultiIoDedicatedFixedTickMetrics metrics;
...

    {
        ...
        options.cadence =
            gamenet::examples::FixedTickCadence::FixedRateBoundedCatchUp;
        ...
        auto profile =
            std::make_unique<gamenet::examples::MultiIoDedicatedFixedTick>(
                &baseLoop,
                logicLoop,
                ...
                [](const gamenet::examples::FixedTickContext&,
                   ...
                   std::string_view payload) {
                    return gamenet::examples::
                        MultiIoDedicatedFixedTickHandlerResult{
        ...
        for (std::size_t clientIndex = 0;
            ...
            clients.emplace_back([&, clientIndex] {
                ...
                while (ok && remaining != 0) {
                    const auto batch =
                        std::min(remaining, config.batchMessages);
    ...
    const bool invariantPassed = clientsSucceeded &&
        ...
        metrics.timerSetupAccepted == 1 &&
        metrics.timerSetupFailures == 0 &&
        metrics.timerCancelAccepted == 1 &&
        metrics.maxConsecutiveCatchUp <= 1 &&
        metrics.crossDomainHandoffs == 2U * messages;

    std::cout << std::fixed << std::setprecision(3)
              << "{\n"
              << "  \"schema\": \"gamenet.multi_io_fixed_tick_benchmark.v1\",\n"
              ...
              << "  \"tick_jitter_p999_us\": "
              << metrics.tickJitterP999Us << ",\n"
              << "  \"tick_duration_p999_us\": "
              << metrics.tickDurationP999Us << ",\n"
              << "  \"queue_age_p999_us\": "
              << metrics.queueAgeP999Us << ",\n"
              << "  \"skipped_ticks\": " << metrics.skippedTicks << ",\n"
              << "  \"catch_up_ticks\": " << metrics.catchUpTicks << ",\n"
              << "  \"tick_overruns\": " << metrics.tickOverruns << ",\n"
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::cerr << "gamenet_multi_io_fixed_tick_benchmark: "
                  << error.what() << '\n';
        return 2;
    }
}
