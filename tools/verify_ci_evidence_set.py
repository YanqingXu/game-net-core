...
EXPECTED_MAIN_INVENTORY = 94
EXPECTED_THREADING_EXECUTION = 67
...
def validate_producer(artifact_dir: Path, manifest_path: Path) -> tuple[str, dict[str, Any], dict[str, Any]]:
    ...
    if job == "linux-tsan":
        require(
            ...
            "TSan inventory does not contain exactly 67 threading tests",
        )
        require(
            inventory_document.get("expected_label_counts", {}).get("threading") == EXPECTED_THREADING_EXECUTION,
            "TSan inventory did not enforce threading=67",
        )
