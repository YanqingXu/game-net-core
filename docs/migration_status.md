...
source-private single-owner Server and zero stable API drift. IOE-X12 then
closed at exact implementation checkpoint `5be30e701c61f8d6700bcc4be6bc0ef152120fb8`
with a source-private multi-owner TCP topology and zero stable API drift.
IOE-X13 then closed at exact implementation checkpoint
`5484d7a89b01597824bc860e4d2d3cf3cfd45a82` with source-private one-shot
...
deferred, and no RPC/Lua Core surface or empty v0.6 release was added. M8 then
audited Core `e5ea9ef` callback/executor/timer semantics, callback-only gateway
`588acd0`, and Actor-bound `YanGameServer@b525416`. Core focused contracts
...
and M10 UDP/KCP evidence review became the governance front. M10 then audited
Core `f1f89f0`, gateway `588acd0`, independent `YanGameServer@b525416`, and
historical `mini_trantor@3eba368`. Both external consumers have no datagram
...
Current M8-G0 async/coroutine audit baseline:
`e5ea9efa71dbe52e841423ec3cac3e9529158b22` (2026-08-24). Gateway
`588acd079be93de3e230ba4f07dd111f7bec6a3c` remains callback/value-only;
exact `YanGameServer@b5254165389d762c3f3c63568c24ffab448fc501` focused
async/coroutine/timer/RPC/persistence evidence passed 9/9 while Core EventLoop/
...
Current M10-G0 UDP/KCP audit baseline:
`f1f89f0e66642b4be3c988400213783e5120536c` (2026-08-24). Gateway
`588acd079be93de3e230ba4f07dd111f7bec6a3c` and exact
`YanGameServer@b5254165389d762c3f3c63568c24ffab448fc501` have no datagram
runtime. Historical source
`mini_trantor@3eba368475a68f677aae920d4f299b155db23d57` contains an old UDP/KCP/
PMTU preview but not the current typed admission, peer generation, retained-
capacity, dual-platform, or installed-package contract. Core SessionManager
...
Profile C cadence-stop summary publication race. IOE-X2 adds the Linux-only,
non-installed `IoUringEventLoopPump`: the ring fd is a readiness scheduling
trigger only, dispatch capacity is independent, and the first EventLoop
Quiescing transition commits cancellation/final-drain work before Shutdown.
IOE-X3 adds one non-installed `IoUringTcpConnectionDriver`: a real loopback TCP
connection proves one Recv in flight, bounded FIFO Send, pause/cancel/no-repost,
re-entry, first-close-reason, and socket release only after Pump physical stop.
...
inventory is 130 configured CTest tests: 8 unit tests, 108 contract tests, and
14 integration tests, with 103 threading and 108 lifecycle labels. Complete local
