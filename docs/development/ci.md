...
`tools/verify_ctest_inventory.py`, requires exactly 113 configured tests, and
writes `ci-evidence/ctest-inventory.json`. The TSan job additionally requires
`threading=86` before it builds and executes that labeled slice. Inventory drift
...
presence: five jobs must execute 113 tests, TSan must execute the exact 86-test
`threading` selection from the 113-test inventory, and Linux Debug plus both
...
python3 tools/verify_ctest_inventory.py --test-dir build-asan --expected-total 113 --output ci-evidence/ctest-inventory.json
...
python3 tools/verify_ctest_inventory.py --test-dir build-tsan --expected-total 113 --expect-label threading=86 --output ci-evidence/ctest-inventory.json
...
python3 tools/verify_ctest_inventory.py --test-dir build-release --expected-total 113 --output ci-evidence/ctest-inventory.json
...
python tools/verify_ctest_inventory.py --test-dir build-windows-release --config Release --expected-total 113 --output ci-evidence/ctest-inventory.json
...
python tools/verify_ctest_inventory.py --test-dir build-asan-windows --config Debug --expected-total 113 --expect-label threading=86
...
113-test inventory, executes the repository guards, installs the package,
...
self-hosted runner: Debug and Release execute all 113 tests plus the installed
package consumer, ASan/UBSan executes all 113 tests, and TSan executes the exact
86-test threading inventory. Every profile verifies the immutable checkout SHA,
...
After configure, `tools/verify_ctest_inventory.py` records the complete 113-test
JSON inventory and requires `threading=86`, `game_pipeline=7`, and
...
python3 tools/verify_ctest_inventory.py --test-dir build-long-soak --expected-total 113 --expect-label threading=86 --expect-label game_pipeline=7 --expect-label broadcast=5 --output long-soak-evidence/ctest-inventory.json
...
python3 tools/verify_ctest_repeat_evidence.py --inventory long-soak-evidence/ctest-inventory.json --log long-soak-evidence/threading-repeat.log --selection-label threading --expected-selected 86 --repeat <repeat> --timeout-seconds <timeout_seconds> --command <exact-command> --output long-soak-evidence/threading-repeat-evidence.json
```
...
python3 tools/verify_ctest_inventory.py --test-dir build --expected-total 113 --output build/ctest-inventory.json
...
cmake -S . -B build-tsan -DCMAKE_BUILD_TYPE=Debug -DGAMENET_BUILD_TESTING=ON -DGAMENET_ENABLE_TSAN=ON
python3 tools/verify_ctest_inventory.py --test-dir build-tsan --expected-total 113 --expect-label threading=86 --output build-tsan/ctest-inventory.json
cmake --build build-tsan --parallel
...
cmake -S . -B build-release -DCMAKE_BUILD_TYPE=Release -DGAMENET_BUILD_TESTING=ON
python3 tools/verify_ctest_inventory.py --test-dir build-release --expected-total 113 --output build-release/ctest-inventory.json
cmake --build build-release --parallel
...
python tools/verify_ctest_inventory.py --test-dir build-windows --config Debug --expected-total 113 --output build-windows/ctest-inventory.json
...
python tools/verify_ctest_inventory.py --test-dir build-windows-release --config Release --expected-total 113 --output build-windows-release/ctest-inventory.json
cmake --build build-windows-release --config Release --parallel
