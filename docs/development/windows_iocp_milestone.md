...
The active M3 line reports
`get_queued_completion_status_ex_batch_64`: Poller storage remains fixed at 64
entries, while `EventLoopOptions::maxIocpCompletionsPerPoll` selects a validated
`[1, 64]` dequeue width. One poll removes at most that many packets, defers
additional packets for the same Channel to a registration-safe later round,
and yields back to the remaining EventLoop phases. Four loopback
