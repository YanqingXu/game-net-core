...
int main() {
    ...
    for (int iteration = 0; iteration < iterationCount; ++iteration) {
        ...
        gamenet::net::TcpServer server(
            ...
            "server-stop-from-worker-callback-soak-" + std::to_string(iteration));
        server.setThreadNum(2);

        std::atomic<int> connectedCallbackCount{0};
        std::atomic<int> disconnectedCallbackCount{0};
        std::atomic<bool> stopIssuedFromWorker{false};
        ...
        server.setConnectionCallback([&](const gamenet::net::TcpConnectionPtr& conn) {
            ...
            if (conn->connected()) {
                ...
                GAMENET_TEST_ASSERT(connected <= clientCount);
                if (connected == clientCount && !stopIssuedFromWorker.exchange(true)) {
                    // server-stop-from-worker-callback-soak: worker-loop
                    // reentrant stop() must marshal base-loop bookkeeping and
                    // still force-close every worker-owned connection.
                    server.stop();
                }
                return;
            }
        ...
        baseLoop.runAfter(std::chrono::seconds(3), [&] {
            GAMENET_TEST_ASSERT(false && "timed out waiting for worker-callback server.stop() teardown");
            baseLoop.quit();
        });
        baseLoop.loop();
        ...
        GAMENET_TEST_ASSERT(stopIssuedFromWorker.load());
