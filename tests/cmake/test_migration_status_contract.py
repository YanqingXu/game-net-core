...
def main() -> None:
    ...
    require(status_text, "Local Windows Debug long-soak smoke", migration_status)
    require(status_text, "ctest --test-dir build -C Debug --output-on-failure -L threading --repeat until-fail:3 --timeout 30", migration_status)
    require(status_text, "36/36 threading-labeled tests passed across 3 repeats", migration_status)
    require(status_text, "remote `long-soak` workflow evidence remains", migration_status)
    require(status_text, "pending until the manual workflow is run on GitHub", migration_status)
    ...
    require(status_text, "56/56 Release tests passed", migration_status)
