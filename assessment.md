...
> Phase 1～3.5 已完成：线程契约、生命周期、CI、长稳测试、跨平台 benchmark 和 Windows IOCP 已收口。Phase 4 尚未正式开始。
...
因此，Phase 3.5 已完成。下一步可以开始 Phase 4，但首个变更仍必须是 PacketFramer 的 design/intent/contracts，不得直接迁移 HTTP、RPC、KCP 或完整 GameServerPipeline。
...
最关键的路线判断没有改变：现在不要迁移 HTTP、RPC、KCP 或完整 GameServerPipeline。Core Preview 已冻结，下一步只从 PacketFramer 切入 Phase 4。这条路线最稳，也最符合你把 `game-net-core` 做成“可长期演进的游戏服务器网络底座”，而不是第二个失控膨胀的 `mini_trantor` 的目标。
